...
{
    public partial class DynamicThreatMap : Visualizer
    {
        ...
        private Dictionary<long, Dictionary<int, int>> _sliceAreaToIncidentPercent;
        private Dictionary<long, Dictionary<int, int>> _sliceIncidentToAreaPercent;
        private Dictionary<long, Dictionary<string, int>> _sliceLocationPercent;
        ...
        private void GetThreatSurfaces(Rectangle bitmapDimensions, bool displayFirstSlice, Dictionary<long, List<Tuple<RectangleF, double, string>>> sliceSquareThreatType = null)
        {
            ...
            Set<string> selectedIncidents = new Set<string>(incidentTypeCheckBoxes.Controls.Cast<ColoredCheckBox>().Where(c => c.CheckState != CheckState.Unchecked && !c.Text.EndsWith(" Highlight")).Select(c => c.Text).ToArray());
            ...
            _sliceAreaToIncidentPercent = new Dictionary<long, Dictionary<int, int>>(slices.Count);
            _sliceIncidentToAreaPercent = new Dictionary<long, Dictionary<int, int>>(slices.Count);
            _sliceLocationPercent = new Dictionary<long, Dictionary<string, int>>(slices.Count);
                ...
                Thread t = new Thread(new ParameterizedThreadStart(core =>
                    {
                        for (int j = (int)core; j < slices.Count; j += Configuration.ProcessorCount)
                        {
                            ...
                            _sliceAreaToIncidentPercent.Add(slice, new Dictionary<int, int>());
                            lock (_sliceIncidentToAreaPercent)
                            _sliceIncidentToAreaPercent.Add(slice, new Dictionary<int, int>());
                            lock (_sliceLocationPercent)
                            _sliceLocationPercent.Add(slice, new Dictionary<string, int>());
                            ...
                            foreach (string location in sortedLocations)
                            {
                                ...
                                lock (_sliceLocationPercent)
                                _sliceLocationPercent[slice].Add(location, areaPercent); 
                                lock (_sliceAreaToIncidentPercent)
                                if (!_sliceAreaToIncidentPercent[slice].ContainsKey(areaPercent))
                                    _sliceAreaToIncidentPercent[slice].Add(areaPercent, incidentPercent);
                                else
                                    _sliceAreaToIncidentPercent[slice][areaPercent] = incidentPercent;
                                lock (_sliceIncidentToAreaPercent)
                                if (!_sliceIncidentToAreaPercent[slice].ContainsKey(incidentPercent))
                                    _sliceIncidentToAreaPercent[slice].Add(incidentPercent, areaPercent);
                ...
                Thread t = new Thread(new ParameterizedThreadStart(core =>
                    {
                        ...
                        using(SolidBrush brush = new SolidBrush(BackColor))
                        {
                            for (int j = (int)core; j < slices.Count; j += Configuration.ProcessorCount)
                            {
                                ...
                                {
                                    List<string> areaPercentLocations =new List<string>();
                                    ...
                                    foreach (int row in sliceRowColScoreIncident[slice].Keys)
                                        ...
                                        {
                                            string location = row + "-" + col;
                                            if (_sliceLocationPercent[slice][location] > percentage)
                                                continue;
                                            areaPercentLocations.Add(location);
                                    ...
                                    foreach (string trueIncidentOverlay in selectedTrueIncidentOverlays)
                                    {
                                        ...
                                        foreach (Incident incident in Incident.Get(sliceStart, sliceEnd, DisplayedPrediction.PredictionArea, trueIncidentOverlay))
                                        {
                                            ...
                                            string location = row + "-" + col;
                                            RectangleF circle = GetCircleBoundingBox(drawingPoint, _pointDrawingDiameter);
                                            if (!areaPercentLocations.Contains(location) || selectedHighlightedIncidents.Contains(trueIncidentHighlightOverlay))
                                            { 
                                            g.FillEllipse(areaPercentLocations.Contains(location) ?highlightBrush:  brush, circle);
                                            g.DrawEllipse(  pen, circle);
                                            }
                                        }
                                    }
        ...
        {
            Invalidate();
            incidentPercentNum.Value = _sliceAreaToIncidentPercent[timeSlice.Value][areaPercent.Value];
            float incidentPercentage = _sliceAreaToIncidentPercent[timeSlice.Value][areaPercent.Value] / 100f;
        ...
        private void incidentPercentNum_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            { 
                ...
                // get area percent
                areaPercentNum.Value = _sliceIncidentToAreaPercent[timeSlice.Value][roundedIncidentPercent];
                // do the rest
                areaPercent.Value = (int)areaPercentNum.Value;
                AdjustCounts();
            }
        ...
        {
            int index = 0;
            Rectangle cropRect = new Rectangle(0, 0, 600, 720);
            ...
            using (StreamWriter sw = new StreamWriter(@"C:\Users\Mohammad\Desktop\questions\exp2\" + textBox1.Text + "\\areaToIncd.txt"))
            {
                foreach (var item in _sliceAreaToIncidentPercent[-1])
                {
                    sw.WriteLine(item.Key + "," + item.Value);
                }
                sw.Close();
            }
            using (StreamWriter sw = new StreamWriter(@"C:\Users\Mohammad\Desktop\questions\exp2\" + textBox1.Text + "\\incdToArea.txt"))
            {
                foreach (var item in _sliceIncidentToAreaPercent[-1])
                {
                    sw.WriteLine(item.Key + "," + item.Value);
                }
                sw.Close();
