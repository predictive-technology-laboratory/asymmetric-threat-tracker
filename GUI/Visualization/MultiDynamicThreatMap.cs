...
{
    public partial class MultiDynamicThreatMap : MultiVisualizer
    {
        ...
        private Dictionary<string, Color> _incidentHighlightColor;
        private Dictionary<string, Color> _predictionColor;
        private List<Dictionary<long, Dictionary<string, List<Tuple<PointF, double>>>>> _sliceIncidentPointScores;
        private Dictionary<long, List<Bitmap>> _sliceThreatSurface;
        private Dictionary<long, List<Dictionary<int, int>>> _sliceAreaToIncidentPercent;
        private Dictionary<long, List<Dictionary<int, int>>> _sliceIncidentToAreaPercent;
        private Dictionary<long, List<Dictionary<string, int>>> _sliceLocationPercent;
        private Dictionary<long, int> totalNumberOfIncidentsPerSlice;
        private int totalNumberOfLocations = -1;
        ...
        private Bitmap CurrentThreatSurface
        {
            get
            {
                List<Bitmap> currentThreatSurfaceList = null;

                if (_sliceThreatSurface != null)
                    _sliceThreatSurface.TryGetValue((int)timeSlice.Value, out currentThreatSurfaceList);

                Bitmap   currentThreatSurface = null;
                if (currentThreatSurfaceList != null)
                    currentThreatSurface = currentThreatSurfaceList[(int)areaPercent.Value];
        ...
        public override void Display(IEnumerable<Prediction> predictions, IEnumerable<Overlay> overlays)
        {
            base.Display(predictions, overlays);
            ...
            foreach (Point p in predictions.First().Points)
                idPoint.Add(p.Id, p);
            ...
            _incidentHighlightColor = new Dictionary<string, Color>();
            _predictionColor = new Dictionary<string, Color>();
            _sliceIncidentPointScores = new List<Dictionary<long, Dictionary<string, List<Tuple<PointF, double>>>>>();
            ...
            foreach (Prediction prediction in predictions)
            {
                 ...
                 if (!_predictionColor.TryGetValue(prediction.Name, out predictionColor))
                 {
                     predictionColor = ColorPalette.GetColor();
                     _predictionColor.Add(prediction.Name, predictionColor);
                 }
                
                DiscreteChoiceModel model = prediction.Model;
                Dictionary<long, Dictionary<string, List<Tuple<PointF, double>>>> predictionIncidentPointScores = new Dictionary<long, Dictionary<string, List<Tuple<PointF, double>>>>();
                foreach (PointPrediction pointPrediction in prediction.PointPredictions)
                {
                    ...
                    predictionIncidentPointScores.EnsureContainsKey(slice, typeof(Dictionary<string, List<Tuple<PointF, double>>>));
                    ...
                    foreach (string incident in pointPrediction.IncidentScore.Keys)
                    {
                        ...
                        string incidentHighlight = incident + " Highlight";
                        if (!_incidentHighlightColor.TryGetValue(incidentHighlight, out color))
                        {
                            color = ColorPalette.GetColor();
                            _incidentHighlightColor.Add(incidentHighlight, color);
                        }

                        double score = pointPrediction.IncidentScore[incident];

                        predictionIncidentPointScores[slice].EnsureContainsKey(prediction.Name, typeof(List<Tuple<PointF, double>>));
                        predictionIncidentPointScores[slice][prediction.Name].Add(new Tuple<PointF, double>(new PointF((float)point.Location.X, (float)point.Location.Y), score));
                    }
            ...
            Invoke(new Action(delegate()
                {
                    ...
                    {
                        ColoredCheckBox cb = new ColoredCheckBox(false, first ? CheckState.Checked : CheckState.Unchecked, incidentType, _incidentColor[incidentType]);
                    ...
                    foreach (string incidentType in _incidentColor.Keys)
                    {
                        string incidentHighlightType = incidentType + " Highlight";
                        ColoredCheckBox cb = new ColoredCheckBox(false, first ? CheckState.Checked : CheckState.Unchecked, incidentHighlightType, _incidentHighlightColor[incidentHighlightType]);
                        cb.CheckBoxCheckStateChanged += new EventHandler(IncidentCheckBox_CheckStateChanged);
                        cb.LabelClicked += new EventHandler(IncidentHighlightCheckBox_LabelClicked);
                        incidentTypeCheckBoxes.Controls.Add(cb);
                        first = false;
                    }
                    modelsCheckBoxes.Controls.Clear();
                    baselineModel.Items.Clear();
                    incidentGainCheckBoxes.Controls.Clear();
                    foreach (Prediction prediction in predictions)
                    {
                        string predictionText = prediction.Name;
                        baselineModel.Items.Add(predictionText);
                        ColoredCheckBox cb = new ColoredCheckBox(false,  CheckState.Checked  , predictionText, _predictionColor[predictionText]);
                        cb.CheckBoxCheckStateChanged += new EventHandler(IncidentCheckBox_CheckStateChanged);
                        cb.LabelClicked += new EventHandler(predictionCheckBox_LabelClicked);
                        modelsCheckBoxes.Controls.Add(cb);
                        Label lb = new Label() { Text = String.Format("{0}:0-(0-0%)", predictionText), AutoSize=true};
                        incidentGainCheckBoxes.Controls.Add(lb);
                    }
                    baselineModel.SelectedIndex = 0;
                    ...
                    bool generateThreatSurfaces = threatResolution.Value != predictions.First().PredictionPointSpacing; // changing the threat resolution will generate new threat surfaces, so only do it here if we won't be changing the current resolution value
                    threatResolution.Value = threatResolution.Minimum = predictions.First().PredictionPointSpacing;
        ...
        private void GetThreatSurfaces(Rectangle bitmapDimensions, bool displayFirstSlice, Dictionary<long, List<Tuple<RectangleF, double, string>>> sliceSquareThreatType = null)
        {
            ...
            Set<string> selectedPredictions = new Set<string>(modelsCheckBoxes.Controls.Cast<ColoredCheckBox>().Where(c => c.CheckState == CheckState.Checked ).Select(c => c.Text).ToArray());
            if (!selectedPredictions.Contains(baselineModel.Text))
            {
                int firstSelectedPrediction = 0;
                foreach (string predictionName in _predictionColor.Keys)
                {
                    if (selectedPredictions.Contains(predictionName))
                    {
                        baselineModel.SelectedIndex = firstSelectedPrediction;
                        break;
                    }
                    firstSelectedPrediction++;
                }
            }
            ...
            GetDrawingParameters(bitmapDimensions, out pixelsPerMeter, out threatRectanglePixelWidth);
                ...
                List<long> slices = _sliceIncidentPointScores.First().Keys.OrderBy(s => s).ToList();
                Dictionary<long, List<Dictionary<int, Dictionary<int, Tuple<double, string>>>>> predictionsSliceRowColScoreIncident = new Dictionary<long, List<Dictionary<int, Dictionary<int, Tuple<double, string>>>>>();
                Dictionary<long, List<Bitmap>> newSliceThreatSurface = new Dictionary<long, List<Bitmap>>(slices.Count);
                List<double> predictionsOverallMinScore = new List<double>(); 
                List<double> predictionsOverallMaxScore = new List<double>(); 
                for (int i = 0; i < _sliceIncidentPointScores.Count; i++)
                {
                    predictionsOverallMinScore.Add(double.MaxValue);
                    predictionsOverallMaxScore.Add(double.MinValue);
                }
                _sliceAreaToIncidentPercent = new Dictionary<long, List<Dictionary<int, int>>>(slices.Count);
                _sliceIncidentToAreaPercent = new Dictionary<long, List<Dictionary<int, int>>>(slices.Count);
                _sliceLocationPercent = new Dictionary<long, List<Dictionary<string, int>>>(slices.Count);
                totalNumberOfIncidentsPerSlice = new Dictionary<long, int>();
                ...
                for (int i = 0; i < Configuration.ProcessorCount; ++i)
                {
                    Thread t = new Thread(new ParameterizedThreadStart(core =>
                    {
                        ...
                        {
                            long slice = slices[j]; 
                            List<Dictionary<int, int>> predictionsSliceAreaToIncidentPercent=new List<Dictionary<int, int>>();
                            List<Dictionary<int, int>> predictionsSliceIncidentToAreaPercent = new List<Dictionary<int, int>>();
                            List<Dictionary<string, int>> predictionsSliceLocationPercent = new List<Dictionary<string, int>>();
                            ...
                            try
                            {
                                ...
                                {
                                    newSliceThreatSurface.Add(slice, new List<Bitmap>());
                                    for (int k = 0; k <= 100; k++)
                                    {
                                        newSliceThreatSurface[slice].Add(new Bitmap(bitmapDimensions.Width, bitmapDimensions.Height, PixelFormat.Format16bppRgb565));
                                    }
                                }
                            }
                            ...
                            #endregion
                            List<Dictionary<int, Dictionary<int, Tuple<double, string>>>> predictionsRowColScoreIncident = new List<Dictionary<int, Dictionary<int, Tuple<double, string>>>>();
                            int predictionIndex = 0;
                            var predictionEnumerator = DisplayedPredictions.GetEnumerator();
                            foreach (var predictionIncidentPointScores in _sliceIncidentPointScores)
                            {
                                predictionEnumerator.MoveNext();
                                Prediction DisplayedPrediction = predictionEnumerator.Current;
                                predictionsSliceAreaToIncidentPercent.Add(new Dictionary<int, int>());
                                predictionsSliceIncidentToAreaPercent.Add(new Dictionary<int, int>());
                                predictionsSliceLocationPercent.Add(new Dictionary<string, int>());
                                ...
                                foreach (string incident in predictionIncidentPointScores[slice].Keys)
                                    if (selectedPredictions.Contains(incident))
                                        foreach (Tuple<PointF, double> pointScore in predictionIncidentPointScores[slice][incident])
                                        {
                                ...
                                Dictionary<string, double> locationAvgThreat = new Dictionary<string, double>();
                                ...
                                foreach (int row in rowColIncidentScores.Keys)
                                    foreach (int col in rowColIncidentScores[row].Keys)
                                    {
                                        ...
                                        locationAvgThreat.Add(location, scoreForMostLikelyIncident);
                                ...
                                predictionsRowColScoreIncident.Add(rowColScoreIncident);
                                #endregion
                                #region sort avg location threat and get percentage of area and incidents at each location
                                ...
                                // get true incidents within the current time slice
                                Dictionary<string, int> locationTrueCount = new Dictionary<string, int>();
                                ...
                                foreach (Incident incident in Incident.Get(sliceStart, sliceEnd, DisplayedPrediction.PredictionArea, _incidentColor.Keys.First()))
                                {
                                    ...
                                    GetThreatRectangleRowColumn(drawingPoint, threatRectanglePixelWidth, out row, out col);
                                    string location = row + "-" + col;
                                    if (!locationTrueCount.ContainsKey(location))
                                        locationTrueCount.Add(location, 0);
                                    locationTrueCount[location]++;
                                }
                                ...
                                if (predictionIndex == 0)
                                    lock (totalNumberOfIncidentsPerSlice)
                                        totalNumberOfIncidentsPerSlice.Add(slice, totalTrueIncidents);
                                totalNumberOfLocations = sortedLocations.Count;
                                foreach (string location in sortedLocations)
                                {
                                    ...
                                    int areaPercent = (int)Math.Floor((locationsSurveilled / (float)locationAvgThreat.Count) * 100);
                                    int incidentPercent = (int)Math.Floor((trueIncidentsCaptured / (float)totalTrueIncidents) * 100);

                                    predictionsSliceLocationPercent[predictionIndex].Add(location, areaPercent);

                                    if (!predictionsSliceAreaToIncidentPercent[predictionIndex].ContainsKey(areaPercent))
                                        predictionsSliceAreaToIncidentPercent[predictionIndex].Add(areaPercent, incidentPercent);
                                    else
                                        predictionsSliceAreaToIncidentPercent[predictionIndex][areaPercent] = incidentPercent;


                                    if (!predictionsSliceIncidentToAreaPercent[predictionIndex].ContainsKey(incidentPercent))
                                        predictionsSliceIncidentToAreaPercent[predictionIndex].Add(incidentPercent, areaPercent);
                                ...
                                lock (this)
                                {
                                    if (sliceMinScore < predictionsOverallMinScore[predictionIndex]) predictionsOverallMinScore[predictionIndex] = sliceMinScore;
                                }

                                lock (this)
                                {
                                    if (sliceMaxScore > predictionsOverallMaxScore[predictionIndex]) predictionsOverallMaxScore[predictionIndex] = sliceMaxScore;
                                }
                                #endregion
                                predictionIndex++;
                            }
                            #region store information from thread
                            lock (predictionsSliceRowColScoreIncident)
                            {
                                predictionsSliceRowColScoreIncident.Add(slice, predictionsRowColScoreIncident);
                            }
                            lock (_sliceAreaToIncidentPercent)
                                _sliceAreaToIncidentPercent.Add(slice, predictionsSliceAreaToIncidentPercent);
                            lock (_sliceAreaToIncidentPercent)
                                _sliceIncidentToAreaPercent.Add(slice, predictionsSliceIncidentToAreaPercent);
                            lock (_sliceLocationPercent)
                                _sliceLocationPercent.Add(slice, predictionsSliceLocationPercent);
                            #endregion
                ...
                List<double> scoreRange = new List<double>();
                for (int i = 0; i < predictionsOverallMaxScore.Count; i++)
                {
                    double  scoreRangeValue  = predictionsOverallMaxScore[i] - predictionsOverallMinScore[i];
                    if (scoreRangeValue  == 0)
                        scoreRangeValue  = float.Epsilon;
                    scoreRange.Add(scoreRangeValue);
                }
                ...
                for (int i = 0; i < Configuration.ProcessorCount; ++i)
                {
                    Thread t = new Thread(new ParameterizedThreadStart(core =>
                    {
                        ...
                        using (SolidBrush highlightBrush = new SolidBrush(BackColor))
                        using (SolidBrush brush = new SolidBrush(BackColor))
                        {
                            for (int j = (int)core; j < slices.Count; j += Configuration.ProcessorCount)
                            {
                                ...
                                {
                                    Dictionary<string, List<string>> areaPercentLocations = new Dictionary<string, List<string>>();
                                    Graphics g = Graphics.FromImage(newSliceThreatSurface[slice][percentage]);
                                    g.Clear(BackColor);
                                    int predictionIndex = 0;
                                    var predictionEnum = DisplayedPredictions.GetEnumerator();
                                    
                                    foreach (var sliceRowColScoreIncident in predictionsSliceRowColScoreIncident[slice])
                                    {
                                        predictionEnum.MoveNext();
                                        string predictionName = predictionEnum.Current.Name;
                                        areaPercentLocations.Add(predictionName, new List<string>());
                                        #region threat
                                        foreach (int row in sliceRowColScoreIncident.Keys)
                                            foreach (int col in sliceRowColScoreIncident[row].Keys)
                                            {
                                                string location = row + "-" + col;
                                                if (_sliceLocationPercent[slice][predictionIndex][location] > percentage)
                                                    continue;
                                                areaPercentLocations[predictionName].Add(location);
                                                Tuple<double, string> scoreIncident = sliceRowColScoreIncident[row][col];
                                                double scaledScore = (scoreIncident.Item1 - predictionsOverallMinScore[predictionIndex]) / scoreRange[predictionIndex];
                                                double percentTransparent = 1 - scaledScore;
                                                Color color = _predictionColor[predictionEnum.Current.Name];
                                    ...
                                    Set<string> selectedTrueIncidentOverlays = new Set<string>(incidentTypeCheckBoxes.Controls.Cast<ColoredCheckBox>().Where(c => c.CheckState == CheckState.Checked && !c.Text.EndsWith(" Highlight")).Select(c => c.Text).ToArray());
                                    Set<string> selectedHighlightedIncidents = new Set<string>(incidentTypeCheckBoxes.Controls.Cast<ColoredCheckBox>().Where(c => c.CheckState == CheckState.Checked && c.Text.EndsWith(" Highlight")).Select(c => c.Text).ToArray());
                                    Prediction DisplayedPrediction = DisplayedPredictions.First();
                                    DateTime sliceStart = DisplayedPrediction.PredictionStartTime;
                                    DateTime sliceEnd = DisplayedPrediction.PredictionEndTime;
                                    if (slice != -1)
                                    {
                                    ...
                                    {
                                        string trueIncidentHighlightOverlay = trueIncidentOverlay + " Highlight";
                                        brush.Color = _incidentColor[trueIncidentOverlay];
                                        highlightBrush.Color = _incidentHighlightColor[trueIncidentHighlightOverlay];
                                        ...
                                        foreach (Incident incident in Incident.Get(sliceStart, sliceEnd, DisplayedPrediction.PredictionArea, trueIncidentOverlay))
                                        {
                                            ...
                                            int offset = 0;
                                            List<Tuple<Color, int>> drawingColors = new List<Tuple<Color, int>>();
                                            
                                                foreach (string predictionName in _predictionColor.Keys)
                                                {
                                                    if (areaPercentLocations[predictionName].Contains(location))
                                                    {
                                                        if (!highlighted)// first time
                                                        {
                                                            drawingColors.Add(new Tuple<Color, int>(highlightBrush.Color, -2));
                                                            offset += 3;
                                                        }
                                                        highlighted = true;
                                                        drawingColors.Add(new Tuple<Color, int>(_predictionColor[predictionName], offset));
                                                        offset += 3;
                                                    }
                                                }
                                            if ((!highlighted&&drawingColors.Count==0)||selectedHighlightedIncidents.Contains(trueIncidentHighlightOverlay))
                                                {
                                                    drawingColors.Add(new Tuple<Color, int>(highlighted ? highlightBrush.Color : brush.Color, offset));
                                                    for (int drawingOrder = drawingColors.Count - 1; drawingOrder >= 0; drawingOrder--)
                                                    {
                                                        brush.Color = drawingColors[drawingOrder].Item1;
                                                        circle = GetCircleBoundingBox(drawingPoint, _pointDrawingDiameter + drawingColors[drawingOrder].Item2);
                                                        g.FillEllipse(brush, circle);
                                                    }
                                                    g.DrawEllipse(pen, circle);
                                                }
            ...
            if (_sliceThreatSurface != null)
            {
                foreach (List<Bitmap> threatSurface in _sliceThreatSurface.Values)
                    foreach (Bitmap item in threatSurface)
                        item.Dispose();
            ...
            timeSlice.ValueChanged += new EventHandler(timeSlice_ValueChanged);
                ...
                totalNumberOfLocationsLabel.Text = totalNumberOfLocations.ToString();
                numberOfLocationsCovered.Maximum = totalNumberOfLocations;
            
                totalNumberOfIncidentsLabel.Text = totalNumberOfIncidentsPerSlice[timeSlice.Value].ToString();
                numberOfIncidentsCaptured.Maximum = totalNumberOfIncidentsPerSlice[timeSlice.Value];
        ...
        private void IncidentHighlightCheckBox_LabelClicked(object sender, EventArgs e)
        {
            ColoredCheckBox cb = sender as ColoredCheckBox;
            if (cb == null)
                throw new ArgumentException("Must pass ColoredCheckBox");
            ...
            {
                ColorPalette.ReturnColor(cb.Label.BackColor);
                cb.Label.BackColor = _incidentHighlightColor[cb.Text] = cd.Color;

                if (cb.Checked)
                    GetThreatSurfaces(new Rectangle(0, 0, CurrentThreatSurface.Width, CurrentThreatSurface.Height), false);
            }
        }
        private void predictionCheckBox_LabelClicked(object sender, EventArgs e)
        {
            ColoredCheckBox cb = sender as ColoredCheckBox;
            if (cb == null)
                throw new ArgumentException("Must pass ColoredCheckBox");
            ...
            {
                ColorPalette.ReturnColor(cb.Label.BackColor);
                cb.Label.BackColor = _predictionColor[cb.Text] = cd.Color;
        ...
        {
            DiscreteChoiceModel model = DisplayedPredictions.First().Model;
            if (model is TimeSliceDCM)
            {
                    ...
                    else if (extension == "shp")
                    {
                        ...
                        try
                        {
                            cmd.CommandText = "CREATE TABLE temp (" +
                                              ...
                                              "region GEOMETRY(POLYGON," + DisplayedPredictions.First().PredictionArea.Shapefile.SRID + ")," +
                            ...
                            foreach (long slice in sliceSquareThreatType.Keys)
                            {
                                ...
                                foreach (Tuple<RectangleF, double, string> squareThreatType in sliceSquareThreatType[slice])
                                {
                                    ...
                                    GetPostGisPointFromDrawingPoint(new PointF(left, bottom))}, DisplayedPredictions.First().PredictionArea.Shapefile.SRID);
        ...
        private void examinePredictionsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            else if (!File.Exists(DisplayedPredictions.First().PointPredictionLogPath))
            ...
            else
            {
                ...
                else
                {
                    ...
                    PostGIS.Polygon threatRectangle = new PostGIS.Polygon(new PostGIS.Point[]{
                                                                          new PostGIS.Point(leftMeters, bottomMeters, DisplayedPredictions.First().PredictionArea.Shapefile.SRID),
                                                                          new PostGIS.Point(leftMeters, bottomMeters + widthMeters, DisplayedPredictions.First().PredictionArea.Shapefile.SRID),
                                                                          new PostGIS.Point(leftMeters + widthMeters, bottomMeters + widthMeters, DisplayedPredictions.First().PredictionArea.Shapefile.SRID),
                                                                          new PostGIS.Point(leftMeters + widthMeters, bottomMeters, DisplayedPredictions.First().PredictionArea.Shapefile.SRID),
                                                                          new PostGIS.Point(leftMeters, bottomMeters, DisplayedPredictions.First().PredictionArea.Shapefile.SRID)}, DisplayedPredictions.First().PredictionArea.Shapefile.SRID);

                    DiscreteChoiceModel model = DisplayedPredictions.First().Model;

                    PointPrediction[] pointPredictions = PointPrediction.GetWithin(threatRectangle, DisplayedPredictions.First()).ToArray();
                    ...
                    if (pointPredictions.Length > 0)
                    {
                        ...
                        {
                            IFeatureBasedDCM featureBasedModel = DisplayedPredictions.First().Model as IFeatureBasedDCM;
                        ...
                        try
                        {
                            ...
                            Dictionary<string, Tuple<List<Tuple<string, double>>, List<Tuple<string, string>>>> pointPredictionLog = model.ReadPointPredictionLog(DisplayedPredictions.First().PointPredictionLogPath, logPointIdsToGet);
        ...
        {
            if (DisplayedPredictions.First() != null)
            {
        ...
        private PostGIS.Point GetPostGisPointFromDrawingPoint(System.Drawing.PointF drawingPoint)
        {
            ...
            return new PostGIS.Point(xMeters, yMeters, DisplayedPredictions.First().PredictionArea.Shapefile.SRID);
        }

        private void areaPercent_Scroll(object sender, EventArgs e)
        {
            Invalidate();
            if (_sliceThreatSurface != null) { 
                areaPercentNum.Value = areaPercent.Value;
                AdjustCounts();
            }
        }
        private void AdjustCounts()
        {
            if (_sliceAreaToIncidentPercent != null && baselineModel.SelectedIndex!=-1)
            {
                
                if (_sliceAreaToIncidentPercent[timeSlice.Value][baselineModel.SelectedIndex].Count == 0)
                {
                    incidentPercentNum.Value = 0;
                    totalNumberOfIncidentsLabel.Text = "";
                    numberOfIncidentsCaptured.Value = 0;
                    numberOfLocationsCovered.Value = 0;
                    areaPercentNum.Value = 0;
                    totalNumberOfLocationsLabel.Text = "";
                    var predictionsEnum = DisplayedPredictions.GetEnumerator();
                    foreach (var modelGainLabel in incidentGainCheckBoxes.Controls)
                    {
                        predictionsEnum.MoveNext();
                        ((Label)modelGainLabel).Text = String.Format("{0}:0-(0-0%)", predictionsEnum.Current.Name);
                    }
                }
                else
                {
                    incidentPercentNum.Value = _sliceAreaToIncidentPercent[timeSlice.Value][baselineModel.SelectedIndex][areaPercent.Value];
                    float incidentPercentage = _sliceAreaToIncidentPercent[timeSlice.Value][baselineModel.SelectedIndex][areaPercent.Value] / 100f;
                    numberOfIncidentsCaptured.Value = (int)Math.Ceiling((incidentPercentage * totalNumberOfIncidentsPerSlice[timeSlice.Value]) - 0.00001f);
                    float areaPercentage = areaPercent.Value / 100f;
                    numberOfLocationsCovered.Value = (int)(areaPercentage * totalNumberOfLocations);
                    int predictionIndex = 0;
                    var predictionsEnum = DisplayedPredictions.GetEnumerator();
                    foreach (var modelGainLabel in incidentGainCheckBoxes.Controls)
                    {
                        predictionsEnum.MoveNext();
                        if (predictionIndex == baselineModel.SelectedIndex)
                            ((Label)modelGainLabel).Text = String.Format("{0}:{1}(-%)", predictionsEnum.Current.Name, numberOfIncidentsCaptured.Value);
                        else
                        {
                            float modelIncidentPercentage = _sliceAreaToIncidentPercent[timeSlice.Value][predictionIndex][areaPercent.Value] / 100f;
                            int modelIncidentNumber = (int)Math.Ceiling((modelIncidentPercentage * totalNumberOfIncidentsPerSlice[timeSlice.Value]) - 0.00001f);
                            int netGain = modelIncidentNumber - (int)numberOfIncidentsCaptured.Value;
                            int percentageGain = numberOfIncidentsCaptured.Value==0?0:(int)Math.Round((netGain / (float)numberOfIncidentsCaptured.Value)*100);
                            ((Label)modelGainLabel).Text = String.Format("{0}:{1}({2}{3},{4}{5}%)", predictionsEnum.Current.Name, modelIncidentNumber, netGain < 0 ? "" : "+", netGain, netGain < 0 ? "↓" : "↑", percentageGain);
                        }
                        predictionIndex++;
                    }
                }
        ...
        private void areaPercentNum_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)13 && _sliceThreatSurface != null)
            {
               
                areaPercent.Value = (int)areaPercentNum.Value;
                AdjustCounts();
            }
        } 
        private void incidentPercentNum_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)13 && _sliceThreatSurface != null)
            { 
                // get incident percent
                float incidentPercent = (int)incidentPercentNum.Value / 100f;
                // round to cloest number if incidents
                int roundedIncidentNumber = (int)Math.Round(incidentPercent * totalNumberOfIncidentsPerSlice[timeSlice.Value]);
                // get cloest percent
                int roundedIncidentPercent = (int)((roundedIncidentNumber / (float)totalNumberOfIncidentsPerSlice[timeSlice.Value]) * 100);
                // get area percent
                areaPercentNum.Value = _sliceIncidentToAreaPercent[timeSlice.Value][baselineModel.SelectedIndex][roundedIncidentPercent];
                // do the rest
                areaPercent.Value = (int)areaPercentNum.Value;
                AdjustCounts();
            }
        ...
        private void numberOfIncidentsCaptured_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)13 && _sliceThreatSurface != null)
            {
                // get cloest percent
                int roundedIncidentPercent = (int)(((int)numberOfIncidentsCaptured.Value / (float)totalNumberOfIncidentsPerSlice[timeSlice.Value]) * 100);
                // get area percent
                areaPercentNum.Value = _sliceIncidentToAreaPercent[timeSlice.Value][baselineModel.SelectedIndex][roundedIncidentPercent];
                // do the rest
                areaPercent.Value = (int)areaPercentNum.Value;
        ...
        private void numberOfLocationsCovered_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)13 && _sliceThreatSurface != null)
            {
                
                // get cloest percent
                int roundedLocationPercent = (int)Math.Ceiling((((int)numberOfLocationsCovered.Value / (float)totalNumberOfLocations) * 100) - 0.00001f);
                // get area percent
                areaPercentNum.Value = roundedLocationPercent;
                // do the rest
                areaPercent.Value = (int)areaPercentNum.Value;
                AdjustCounts();
            }
        ...
        private void baselineModel_SelectedIndexChanged(object sender, EventArgs e)
        {
            AdjustCounts();
        }
        ...
        {
            int index = 0;
            Rectangle cropRect = new Rectangle(0, 0, 710, 850);
            foreach (var image in _sliceThreatSurface[-1])
            {
                // image.Save(@"C:\Users\Mohammad\Desktop\questions\exp2\" + textBox1.Text + "\\" + (index++) + ".jpg", ImageFormat.Jpeg);
                ...
                target.Save(@"C:\Users\Mohammad\Desktop\questions\exp2\" + textBox1.Text + "\\" + (index++) + ".png", ImageFormat.Png);
            }
            for (int i = 0; i < _sliceAreaToIncidentPercent[-1].Count; i++)
            {
                using (StreamWriter sw = new StreamWriter(@"C:\Users\Mohammad\Desktop\questions\exp2\" + textBox1.Text + "\\areaToIncd_"+i+".txt"))
                {
                    foreach (var item in _sliceAreaToIncidentPercent[-1][i])
                    {
                        sw.WriteLine(item.Key + "," + item.Value);
                    }
                    sw.Close();
                }
                using (StreamWriter sw = new StreamWriter(@"C:\Users\Mohammad\Desktop\questions\exp2\" + textBox1.Text + "\\incdToArea_" + i + ".txt"))
                {
                    foreach (var item in _sliceIncidentToAreaPercent[-1][i])
                    {
                        sw.WriteLine(item.Key + "," + item.Value);
                    }
                    sw.Close();
